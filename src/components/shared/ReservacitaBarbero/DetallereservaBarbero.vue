<template>
  <div class="detalle-wrapper">
    <div class="detalle-card" :style="{ background: cardBg, border: `1.5px solid ${cardBorder}` }">
      <!-- Header barbería -->
      <div class="detalle-header" :style="{ background: isDark ? '#161d2f' : '#1a1a1a' }">
        <div class="header-logo-wrap">
          <v-avatar size="50" class="header-logo" :style="{ border: `2px solid ${isDark ? 'rgba(238, 111, 56, 0.4)' : 'rgba(238, 111, 56, 0.3)'}` }">
            <v-img src="/public/imagenes/logo/logo2.png" alt="Logo"></v-img>
          </v-avatar>
        </div>
        <div class="header-info">
          <span class="header-nombre" :style="{ color: '#fff' }">StyleHub</span>
          <span class="header-dir" :style="{ color: 'rgba(255,255,255,0.6)' }">
            <i class="fas fa-map-marker-alt mr-1"></i>
            Calle 30 Cr 4 - n° 3 - Local #5
          </span>
        </div>
        <div class="header-badge" :style="{ background: 'rgba(238, 111, 56, 0.15)', border: '1px solid rgba(238, 111, 56, 0.3)' }">
      ...
      <div class="detalle-contenido">
        ...
        <div class="seccion mb-6">
          ...
          <div v-else class="seccion-vacia" :style="{ background: innerBg, borderColor: cardBorder }">
            <i class="fa-regular fa-calendar-plus" :style="{ color: isDark ? '#374151' : '#ccc' }"></i>
            <p :style="{ color: txtSecondary }">Sin servicios seleccionados</p>
          </div>
        </div>

        <div class="divider" :style="{ background: cardBorder }"></div>

        <!-- Profesional (pre-seleccionado) -->
        <div class="seccion my-6">
          <div class="seccion-header">
            <div class="seccion-icon" :style="{ background: isDark ? 'rgba(238, 111, 56, 0.15)' : 'rgba(238, 111, 56, 0.1)' }">
          ...
          <div v-if="barbero" class="barbero-preview" :style="{ background: innerBg }">
            ...
            <div class="barbero-preview-datos ml-3">
            ...
            <!-- Chip pre-seleccionado -->
            <div class="preseleccionado-chip" :style="{ background: isDark ? 'rgba(46, 125, 50, 0.15)' : 'rgba(46, 125, 50, 0.1)', color: isDark ? '#81c784' : '#2e7d32', borderColor: isDark ? 'rgba(46, 125, 50, 0.25)' : 'rgba(46, 125, 50, 0.2)' }">
...
<script setup>
  ...
  const formatPrecio = (precio) => {
    ...
    const p = typeof precio === 'string' ? precio : String(precio)
...
<style scoped>
  ...
  .divider { height: 1px; margin: 14px 0; opacity: 0.5; }
  ...
  .seccion-vacia i { font-size: 32px; margin-bottom: 12px; }
  .seccion-vacia p { font-size: 0.85rem; font-weight: 600; margin: 0; }

  .barbero-preview { display: flex; align-items: center; padding: 14px; border-radius: 14px; position: relative; }
